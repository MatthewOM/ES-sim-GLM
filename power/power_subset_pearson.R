...
for (i in 1:length(trees.sample.50)) {
  ...
  essim.res <- essim(trees.sample.50[[i]], trait, nsim = 1000)
  essim.p <- c(essim.p, essim.res[2])
  
}

essim.pow.50 <- length(essim.p[essim.p < 0.05])/length(trees.sample.50)

write.csv(essim.pow.50, "power.50.pearson.subset.csv")
...
for (i in 1:100) {
  dtips <- sample(trees1250sdd[[i]]$tip.label, 1150)
...
for (i in 1:length(trees.sample.100)) {
  ...
  essim.res <- essim(trees.sample.100[[i]], trait, nsim = 1000)
  essim.p <- c(essim.p, essim.res[2])
  
}

essim.pow.100 <- length(essim.p[essim.p < 0.05])/length(trees.sample.100)

write.csv(essim.pow.100, "power.100.pearson.subset.csv")
...
for (i in 1:length(trees.sample.250)) {
  ...
  essim.res <- essim(trees.sample.250[[i]], trait, nsim = 1000)
  essim.p <- c(essim.p, essim.res[2])
  
}

essim.pow.250 <- length(essim.p[essim.p < 0.05])/length(trees.sample.250)

write.csv(essim.pow.250, "power.250.pearson.subset.csv")
